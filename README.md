...
95% bootstrap confidence bands. It separately evaluates one seed's *final*
policy (DQN greedy, no further learning; the random policy is stationary
throughout) for at least 100 episodes each, and plots the resulting
state-occupancy heatmaps alongside the learning curves.

![DQN vs. random agent on Pinball easy: learning curves and final-policy state occupancy](benchmark_dqn.png)
