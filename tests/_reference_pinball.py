"""Vendored numpy reference Pinball physics, used ONLY as a test oracle.

This is a verbatim copy of the original numpy ``PinballModel`` implementation
that ``pinball_jax`` is a JAX translation of. The parity tests roll out both
side by side and assert the JAX env matches this reference.
...
class PinballModel:
    ...
    def __init__(self, configuration, rng: np.random.Generator):
        """read a configuration file for Pinball and draw the domain to screen
    ...
    def reset_ball_to_start_state(self):
        self.ball = BallModel(list(self.rng.choice(self.start_pos)), self.ball_rad)
